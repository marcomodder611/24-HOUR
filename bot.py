<html>
 <head></head>
 <body>
  import datetime import random import time import requests import streamlit as st # --- API &amp; TELEGRAM CONFIGURATION --- TELEGRAM_API_KEY = "8309364556:AAGZZM4B0hmAQU-7jYd9x6e1w1wVzyGg-Ck" TELEGRAM_CHAT_ID = "-1004405838356" def send_telegram_alert(message): """Sends notification updates to the specified Telegram chat.""" url = f"https://api.telegram.org/bot{TELEGRAM_API_KEY}/sendMessage" payload = {"chat_id": TELEGRAM_CHAT_ID, "text": message, "parse_mode": "HTML"} try: requests.post(url, json=payload, timeout=5) except Exception: pass # Fail silently to avoid interrupting the main thread if offline # --- PAGE CONFIGURATION --- st.set_page_config( page_title="VIP RAJPUT • 2-LEVEL PATTERN ENGINE", page_icon="👑", layout="centered", ) # --- THEME STYLING CONFIGURATIONS --- THEMES = { "👑 Royal Gold": { "bg": "#0a0a1a", "logo": "#FFD700", "card_bg": "#0d0d2b", "accent": "#00FF88", "border": "#FFD70044", }, "🔥 Crimson Warrior": { "bg": "#0a0000", "logo": "#FFD700", "card_bg": "#1a0000", "accent": "#FF4500", "border": "#FF450044", }, "🔮 Purple Emperor": { "bg": "#0a0015", "logo": "#FFD700", "card_bg": "#15002a", "accent": "#9B59B6", "border": "#9B59B644", }, "💎 Emerald King": { "bg": "#000a00", "logo": "#FFD700", "card_bg": "#001a00", "accent": "#00FF88", "border": "#00FF8844", }, "🌹 Rose Gold Luxury": { "bg": "#0a0008", "logo": "#FFD700", "card_bg": "#1a0015", "accent": "#E8A0BF", "border": "#E8A0BF44", }, } # --- SESSION STATE INITIALIZATION --- if "history" not in st.session_state: st.session_state.history = [] if "wins" not in st.session_state: st.session_state.wins = 0 if "losses" not in st.session_state: st.session_state.losses = 0 if "jackpots" not in st.session_state: st.session_state.jackpots = 0 if "last_period" not in st.session_state: st.session_state.last_period = "" if "alert_sent" not in st.session_state: st.session_state.alert_sent = False # --- UI HEADER &amp; THEME SWITCHER --- st.markdown( "
  ...
  ", unsafe_allow_html=True, ) selected_theme_name = st.selectbox( "Select Theme", list(THEMES.keys()), label_visibility="collapsed" ) theme = THEMES[selected_theme_name] # Apply Theme Style via CSS injection st.markdown( f"""
  <style>
  ...
  """, unsafe_allow_html=True, ) # --- TIME &amp; PERIOD ENGINE LOGIC --- now = datetime.datetime.utcnow() total_minutes = now.hour * 60 + now.minute period_str = f"{now.strftime('%Y%m%d')}1000{10001 + total_minutes}" remaining_seconds = 60 - now.second col_p1, col_p2 = st.columns(2) with col_p1: st.markdown( f"""
  <div class="metric-card">
   <div style="font-size: 11px; color: #888;">CURRENT PERIOD</div>
   <div style="font-size: 22px; font-weight: 900; color:" f_ {theme[_logo_]};_>{period_str}</div>
  </div>
  """, unsafe_allow_html=True, ) with col_p2: st.markdown( f"""
  <div class="metric-card">
   <div style="font-size: 11px; color: #888;">TIMER</div>
   <div style="font-size: 26px; font-weight: 900; color:" f_ {theme[_accent_]};_>00:{remaining_seconds:02d}</div>
  </div>
  """, unsafe_allow_html=True, ) st.markdown("
  <br>
  ", unsafe_allow_html=True) # --- 2-LEVEL PATTERN ENGINE PREDICTION LOGIC --- if st.session_state.last_period != period_str: pred_type = random.choice(["BIG", "SMALL"]) pred_num = random.randint(0, 4) if pred_type == "BIG" else random.randint(5, 9) confidence = random.randint(88, 98) level = random.choice(["LEVEL 1 [PRIMARY]", "LEVEL 2 [RECOVERY]"]) if st.session_state.last_period != "": actual_res = random.choice(["BIG", "SMALL"]) is_jackpot = random.random() &lt; 0.15 if is_jackpot: status = "JACKPOT" st.session_state.jackpots += 1 st.session_state.wins += 1 elif pred_type == actual_res: status = "WIN" st.session_state.wins += 1 else: status = "LOSS" st.session_state.losses += 1 st.session_state.history.insert( 0, { "period": st.session_state.last_period, "pred": pred_type, "status": status, }, ) st.session_state.current_pred = pred_type st.session_state.current_num = pred_num st.session_state.confidence = confidence st.session_state.level = level st.session_state.last_period = period_str st.session_state.alert_sent = False # Send Telegram Alert at the start of every new period cycle if not st.session_state.alert_sent: telegram_msg = ( f"👑 <b>ROHIT • 2-LEVEL PATTERN ENGINE</b>\n\n" f"📌 <b>Period:</b> {period_str}\n" f"📊 <b>Level:</b> {st.session_state.level}\n" f"🎯 <b>Prediction:</b> <b>{st.session_state.current_pred}</b> ({st.session_state.current_num})\n" f"⭐ <b>Confidence:</b> {st.session_state.confidence}%\n\n" f"🚀 <i>Place your bets now!</i>" ) send_telegram_alert(telegram_msg) st.session_state.alert_sent = True # --- PREDICTION DISPLAY CARD --- st.markdown( f"""
